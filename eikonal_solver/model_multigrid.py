...
def _eikonal_soft_sweeping_diff_init(
    cost: torch.Tensor,
    src_mask: torch.Tensor,
    cfg: "EikonalConfig",
    T_init: torch.Tensor,
    ...
    This is a multigrid / warm-start variant of `_eikonal_soft_sweeping_diff`.
    Instead of initialising T with `large_val` everywhere, it starts from `T_init`.

    IMPORTANT NOTE about monotone updates:
      - When cfg.monotone=True, T can only decrease (via min / softmin), so T_init
        MUST be an upper bound of the true solution; otherwise the solver may get
        stuck at an underestimated T.
      - For coarse-to-fine warm-start, the coarse solution can underestimate the
        fine solution (due to discretisation / max-pooling), so it is recommended
        to use cfg.monotone=False for the fine refinement stage.

    Args:
        cost:        [B,H,W] or [H,W]
        src_mask:    [B,H,W] or [H,W] bool
        cfg:         EikonalConfig (n_iters, h, mode, monotone, ...)
        T_init:      same shape as cost, warm-start field
        checkpoint_chunk: gradient checkpoint segment length
        gate_alpha:  residual gate in iterative updates
    """
    squeeze_back = cost.dim() == 2
    if squeeze_back:
        cost = cost.unsqueeze(0)
    ...
    # [MOD] start from provided initial field
...
class SAMRoute(SAMRoad):
    ...
    def __init__(self, config):
        ...
        # --- Multigrid warm-start config ---
        self.route_multigrid         = bool(_cfg(config, "ROUTE_MULTIGRID",       False))
        self.route_mg_factor         = int(_cfg(config,  "ROUTE_MG_FACTOR",       4))
        self.route_mg_iters_coarse   = int(_cfg(config,  "ROUTE_MG_ITERS_COARSE", 0))
        self.route_mg_iters_fine     = int(_cfg(config,  "ROUTE_MG_ITERS_FINE",   0))
            ...
            with torch.no_grad():
                if tgt_yx.dim() == 3:
                    ...
                    if self.route_multigrid:
                        ...
                        itf = self.route_mg_iters_fine   if self.route_mg_iters_fine   > 0 else max(20, int(n * 0.80))
                        pred_dist_raw = self._roi_multi_target_multigrid_diff_solve(
                            road_prob, src_yx, tgt_yx, gt_dist,
                            mg_factor=self.route_mg_factor,
                            mg_iters_coarse=itc,
                            mg_iters_fine=itf,
                            mg_detach_coarse=self.route_mg_detach_coarse,
    ...
    def _roi_multi_target_multigrid_diff_solve(
        self,
        road_prob: torch.Tensor,    # [B, H, W]
        src_yx: torch.Tensor,       # [B, 2] long
        tgt_yx_k: torch.Tensor,     # [B, K, 2] long
        gt_dist_k: torch.Tensor,    # [B, K] float  — <=0 for padding entries
        eik_cfg: "EikonalConfig | None" = None,
        *,
        mg_factor: int = 4,
        ...
        Args:
            ...
            mg_iters_coarse:  iterations on coarse grid
            mg_iters_fine:    refinement iterations on fine grid
        ...
        # -------------------------
        actual_iters_coarse = int(max(mg_iters_coarse, int(P_c2 * 1.5)))
        cfg_c = _replace(
            cfg,
            h=float(ds_coarse),
            n_iters=actual_iters_coarse,
            monotone=True,
        )
        ...
        # upsample coarse T to fine grid
        mg_interp = str(mg_interp).lower()
        if mg_interp not in ("nearest", "bilinear"):
            mg_interp = "bilinear"

        if mg_interp == "nearest":
            T_init = F.interpolate(T_c.unsqueeze(1), size=(P_c, P_c), mode="nearest").squeeze(1)
        else:
