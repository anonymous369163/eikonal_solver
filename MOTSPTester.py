...
class TSPTester:
    ...
    def run(self, shared_problem, pref, episode=0, distance_matrix=None):
        ...
        while episode < test_num_episode:
            ...
            aug_score = self._test_one_batch(shared_problem, pref, batch_size, episode, distance_matrix)
    ...
    def _test_one_batch(self, shared_probelm, pref, batch_size, episode, distance_matrix=None):
        ...
        # Get distance matrix for this batch if provided
        dist_mat = None
        if distance_matrix is not None:
            dist_mat = distance_matrix[episode: episode + batch_size]

        self.env.load_problems(batch_size, aug_factor, problems, distance_matrix=dist_mat)
        ...
        # Handle single-objective case: obj2 = obj1 if only 1 objective
        if reward.size(-1) > 1:
            max_reward_obj2 = rearrange(reward[:,:,1].reshape(aug_factor, batch_size, self.env.pomo_size), 'c b h -> b (c h)').gather(1, max_idx_aug)
        else:
            max_reward_obj2 = max_reward_obj1  # Single-objective: duplicate obj1
